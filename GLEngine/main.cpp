...
uint32_t verticiesLength = sizeof(verticies) / sizeof(Vertex);
uint32_t indicesLength = sizeof(indices) / sizeof(uint32_t);
...
int main()
{
	...
	while (!window.GetIsClosed())
	{
		...
		//glDrawArrays(GL_TRIANGLES, 0, verticiesLength);
